...
test('current EER, goal adjustment, macros, and body metrics',()=>{const p=profile(seed.profile);assert.ok(Math.abs(p.energy-2234.54)<1e-8);assert.ok(Math.abs(p.target-1734.54)<1e-8);assert.equal(p.bmi_label,'Sobrepeso');assert.ok(Math.abs(p.macros.protein.grams-88.4)<1e-8);const measured=profile({...seed.profile,body_fat:30,waist:90,target_weight:60});assert.ok(Math.abs(measured.lean_mass-47.6)<1e-8);assert.equal(measured.waist_height_label,'Adiposidade central aumentada');assert.ok(Math.abs(measured.macros.protein.grams-78)<1e-8);assert.ok(Math.abs(profile({...seed.profile,adjustment:0}).target-p.energy)<1e-8);});
