...
<template>
<div><p v-if="error" class="error-banner" role="alert">{{error}}</p><div class="section-head"><label>Buscar paciente<input v-model="search" placeholder="Nome do paciente" /></label><button class="button primary" @click="edit()">Cadastrar paciente</button></div>
...
<style scoped>.patient-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:18px;margin-top:24px}.patient-list p{overflow-wrap:anywhere}.patient-actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:16px}</style>
